...
<body>
<h4>Http Reqest(요청) => HttpServletRequest </h4>
<a href="?param1=sdfds&param2=dfdf">A 태그를 이용한 GET 요청</a>
<form method="post" >
...
<pre>
	...
	1. Request Line (수신자) : URL(URI)-명사(자원에 대한 식별자), request Method(동사, 행위정보, CRUD...)
	   <%=request.getRequestURI() %>, <%=request.getMethod() %>
 	   ...
 	   3) PUT(Update) : 서버 자원의 모든 속성을 수정하기 위한 요청 .
 	   4) PATCH(Update) : 서버 자원의 특정 속성을 수정하기 위한 요청.
 	   5) DELETE : 서버의 자원 삭제.
 	   6) OPTIONS : pre-flight 요청에 사용되는 메소드.
	...
	3. Request Body (편지지) : Content Body, Message Body, only POST 요청에서만 형성.
	   ...
	   1. request parameter
		   ...
		   2) get request : line 을 통해 url의 query string 형태로 전송. - req.getQueryString, req.getParameter*..
		   
	   2. multipart request : body 를 통해 전송, body 를 여러개의 part로 분리하여 전송. - req.getPart, req.getParts
