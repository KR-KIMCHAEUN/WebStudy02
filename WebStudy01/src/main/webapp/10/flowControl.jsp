...
<h4>페이지 이동 구조</h4>
<pre>
<!--두 개의 차이 : 기존의 req을 유지하느냐 하지 않느냐  -->
	Http : ConnectLess. StateLess

	1.요청 분기(request dispatch) : 원본 요청을 가지고 분기하는 서버사이드 이동 구조.(RequestDispatcher 사용)
		1) forward : request 와 response의 처리자가 분리되는 구조(Model2 에서 주로 활용).
		2) include : 최종 응답의 형태가 2개 이상의 jsp 파일이 한개의 페이지를 구성하는 형태로 응답 전송(A+B, 페이지 모듈화 구조). 
		...
		<%
			...
			String path = "/02/standard.jsp"; //서버사이드 경로로 표현하기 때문에 컨텍스트 패스 필요 x
			RequestDispatcher rd = request.getRequestDispatcher(path);
// 			rd.forward(request,response);
// 			rd.include(request,response); //페이지 모듈화 하나의 도큐먼트에 2개의 jsp 표현
// 			pageContext.include(path);
		%>
		<jsp:include page="<%=path%>" /> 
	2. Redirect : body가 없는 3xx 응답이 전송되면서, 원본 요청이 사라지고(StateLess), 
