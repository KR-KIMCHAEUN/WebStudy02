...
/**
 * reflection : 객체(instance)로 부터 객체의 타입, 속성, 메소드 등의 조건(구조)를 역으로 분해하는 작업.
 *
 */
@Slf4j
@TestMethodOrder(MethodOrderer.MethodName.class) // single value 어노테이션
...
//         Method setter =  targetType.getDeclaredMethod("setMemId",String.class);
         ...
         Method getter = pd.getReadMethod(); // getter는 argument x
...
//         Method setter =  targetType.getDeclaredMethod("setMemId",String.class);
   ...
   void test5() throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
      String value = BeanUtils.getProperty(target, "memId"); // 리플렉션과 자바 빈 사용
      log.info("{}",value);
      BeanUtils.setProperty(target, "memId", "임의변경값");
      value = BeanUtils.getProperty(target, "memId"); // target -> 객체
      log.info("{}",value);
   }
