...
<pre>
	1. session : 상태 유지를 위한 정보가 서버측에 저장되는 개념.
		세션(HttpSession) 
			database - connectFull --> connection == session (통로)
			http - connectLess --> 시간(웹앱을 사용하기 시작하는 순간부터 종료 이벤트가 발생할 때까지의 기간)의 의미로 세션을 표현함.
		세션의 생명주기
			생성 : 한 클라이언트가 사용하고 있는 하나의 에이전트(브라우저)에 대해 최초의 요청이 발생했을 때 세션이 생성됨.
				  각 세션은 식별자로 세션 아이디를 발급 받음. 
			...
			소멸 : 만료 시간(timeout) 이내에 새로운 요청이 발생하지 않는 경우.
				...
				<%
					//session.invalidate();
				%>
	2. cookie : 성태 유지를 위한 정보가 클라이언트에 저장되는 개념.
