...
<body>
<h4>서버에서 클라이언트로 전송된는 응답 형태 : HttpServletResponse</h4>
<pre>
	1.Response Line ; status code
		...
		3) 300~	: response body가 없기 때문에 클라이언트가 응답을 받은 후 추가 액션을 행하게 됨.
			304 : Not Modified(<%=HttpServletResponse.SC_NOT_MODIFIED %>) , 캐시 영역 검색  --  레이턴시 타임 
			302/307 : Moved, 자원의 새로운 위치를 대상으로 새로운 요청을 전송함.  
			
		4) 400~	: client side error
			400	: <%=HttpServletResponse.SC_BAD_REQUEST %> : 필수 파라미터 누락, 요청 데이터 타입 문제, 요청 데이터 길이가 너무 길 때.. (요청 검증시 의도적으로 전송)
			401/403 : <%=HttpServletResponse.SC_UNAUTHORIZED%> / <%=HttpServletResponse.SC_FORBIDDEN %> : 보안(인증(401)+인가(403,권한을 확인)) 처리를 위한 접근 제한
			404 : <%=HttpServletResponse.SC_NOT_FOUND %> : URL 오류 시(데이터가 소유하지 않은 데이터를 요구할 때 ) 
			405	: <%=HttpServletResponse.SC_METHOD_NOT_ALLOWED %> : 서블릿에 정의되지 않은 메소드로 요청할 때
